/**
 * @author Kenta Xu w/ help from Alvin Li in Office Hours
 */
...
void cachesim_init(int _block_size, int _cache_size, int _ways) {
    ...
    for(int i= 0 ; i< num_sets; i++) {
        ...
        for (int j = 0; j < ways; j++) {  //initialize all bits/tags to 0
...
void cachesim_access(addr_t physical_addr, int access_type) {
    ...
    accesses++;
    int insert = lru_stack_get_lru(cache[index].stack); //LRU index

    //CASE FOR IF TAG IS FOUND AND VALID
    for (int i = 0; i < ways; i ++) {
        if (tag == cache[index].blocks[i].tag && cache[index].blocks[i].valid == 1) {
            hits++;
            if (access_type == MEMWRITE) {
                cache[index].blocks[i].dirty = 1; //set dirty bit to 1 only on MEMWRITE
            }
            lru_stack_set_mru(cache[index].stack, i); //updates LRU stack with index of way
            return;
        }
    }

    //CASE FOR IF NOT VALID OR NO TAG
    misses++;

    //case for writeback: when miss and dirty bit/valid 1
    if (cache[index].blocks[insert].dirty == 1 && cache[index].blocks[insert].valid == 1) { //condition for writeback
        writebacks++;
    }
    cache[index].blocks[insert].tag = tag;  //new tag
    cache[index].blocks[insert].valid = 1;  //make sure valid
    if (access_type == MEMWRITE) {
        cache[index].blocks[insert].dirty = 1; //dirty bit 1 only on MEMWRITE
    } else {
        cache[index].blocks[insert].dirty = 0;
    }
    lru_stack_set_mru(cache[index].stack, insert); //updates LRU stack with index of way
    ////////////////////////////////////////////////////////////////////
    //  End of your code
    ////////////////////////////////////////////////////////////////////
}
